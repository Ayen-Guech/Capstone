...
const SRHDashboard: React.FC = () => {
  ...
  const openSource = async (url: string) => {
    ...
    try {
      ...
      setSourceData({ error: err.message });
    }
  };
          ...
          {sourceData && !sourceData.error && (
            <>
              ...
              {Array.isArray(sourceData.summary) && (
                <>
                  <h6 style={{ color: "#B00059", fontWeight: 600 }}>Summary:</h6>
                  {sourceData.summary.map((p, i) => (
...
/* ------------------------------------------------
    Dynamic Topic-Based Literacy Check
------------------------------------------------ */
...
const ProtectedAccess: React.FC<ProtectedProps> = ({ sourceUrl, topic }) => {
  ...
  // Dynamic comprehension questions based on article topic
  const topicQuestions = (topic: string) => {
    ...
    } else if (lower.includes("hiv") || lower.includes("sti")) {
      return [
        {
